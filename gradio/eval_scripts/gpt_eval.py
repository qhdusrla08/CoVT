...
PAIR_JSONL = os.path.join(EVAL_DIR, "pairs.jsonl")
EVAL_JSONL = os.path.join(EVAL_DIR, "eval_results.jsonl")
SCORES_CSV = os.path.join(EVAL_DIR, "win_rates.csv")
...
def parse_model_prompt_from_filename(fname: str) -> Optional[Tuple[str, str]]:
    m = re.match(r"^captions_(base|covt)_(P\d+)\.jsonl$", fname)
    if not m:
        return None
...
# =========================
# GPT blind A/B (B)  (swapped is handled ONLY here)
# =========================
JUDGE_SYSTEM = (
