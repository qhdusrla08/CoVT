...
def extract_visual_thought(text: str) -> str:
    # <think> 태그 내의 텍스트와 시각적 토큰을 모두 추출합니다. 
...
def run_single_inference(
    ...
    # skip_special_tokens=False로 설정해야 <|sam_pad|> 같은 시각적 토큰이 보임
    raw = processor.decode(new_tokens, skip_special_tokens=False) 
    
    answer = extract_caption(raw)
    thought = extract_visual_thought(raw) # 사고 과정 추출 추가

    return answer, thought, elapsed # thought를 함께 반환
...
def main():
    ...
    for idx, (track, image_id, image_path) in enumerate(items, 1):
        ...
        try:
            caption, thought, elapsed = run_single_inference(
                model=model,
                processor=processor,
                image_path=image_path,
                question=prompt,
            ...
            rec["caption"] = caption
            rec["thought"] = thought  # 시각적 사고 과정 필드 추가
